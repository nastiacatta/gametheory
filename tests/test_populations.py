...
def test_homogeneous_recency_uses_sampled_banks() -> None:
    ...
    assert len(agents) == 5
    for agent in agents:
        assert isinstance(agent, RecencyWeightedPredictorAgent)
        assert len(agent.predictor_names) == 6
...
def test_build_heterogeneous_uses_requested_shares() -> None:
    ...
    assert sum(isinstance(a, BestPredictorAgent) for a in agents) == 101


def test_build_heterogeneous_rejects_invalid_share_sum() -> None:
    with pytest.raises(ValueError, match="must equal 1.0"):
        build_heterogeneous(
            101, p_best=0.3, p_softmax=0.3, p_random=0.6,
...
def test_build_homogeneous_best_predictor() -> None:
    """Test the new best predictor builder."""
    agents = build_homogeneous_best_predictor(
        n_players=10, predictors_per_agent=6, seed=42
    )
...
def test_build_homogeneous_softmax() -> None:
    """Test the new softmax predictor builder."""
    agents = build_homogeneous_softmax(
        n_players=10, beta=2.0, predictors_per_agent=6, seed=42
    )
    assert len(agents) == 10
    for agent in agents:
